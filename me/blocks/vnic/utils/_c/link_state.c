...
#include <assert.h>
#include <nfp/xpb.h>
#include <vnic/utils/link_state.h>
#include <vnic/utils/nfp_mac.h>
...
mac_eth_port_link_state(unsigned int mac_isl, unsigned int eth_port)
{
    ...
    eth_status = xpb_read(NFP_MAC_XPB_OFF(mac_isl)
                          | NFP_MAC_GLBL_MAP_MAC_ETH(mac_core)
                          | NFP_MAC_ETH_MAC_SEG(mac_core_port)
                          | NFP_MAC_ETH_MAC_SEG_ETH_STATUS);

    if (!(eth_status
          & (NFP_MAC_ETH_MAC_SEG_ETH_STATUS_ETH_PHY_LOS
             | NFP_MAC_ETH_MAC_SEG_ETH_STATUS_ETH_RX_REMOTE_FAULT
             | NFP_MAC_ETH_MAC_SEG_ETH_STATUS_ETH_RX_LOCAL_FAULT))) {
        link_state = LINK_UP;
    }
