            ...
            size_t size, size_t max_size, sync_t sync, SIGNAL *sig)
{
    ...
    _PCIE_C2P_CMD(read, data, pcie_isl, bar_idx, addr_hi, addr_lo, \
                  size, max_size, sync, sig);
}
             ...
             size_t size, size_t max_size, sync_t sync, SIGNAL *sig)
{
    ...
    _PCIE_C2P_CMD(write, data, pcie_isl, bar_idx, addr_hi, addr_lo, \
                  size, max_size, sync, sig);
}
...
__intrinsic void
pcie_dma_set_sig(void *cmd,
                 unsigned int meid, unsigned int ctx, unsigned int sig_no)
{
    struct nfp_pcie_dma_cmd *cmd_ptr;
    unsigned int mode_msk = ((1<<31 - 1) - (NFP_PCIE_DMA_CMD_DMA_MODE_shf - 1));
    unsigned int tmp = (((meid & 0xF) << 13) | (((meid>>4) & 0x3F) << 7) |
                        ((ctx & 0x7) << 4) | sig_no);
    ...
    /* NB: Alternative code using structures is beaten by ~3 cycles */
    /* cmd_ptr->mode_sel = ((meid >> 3) & 0x1); */
    /* cmd_ptr->dma_mode = (((meid & 0x7) << 13) | (((meid>>4) & 0x3F) << 7) |*/
    /*                      ((ctx & 0x7) << 4) | sig_no); */
}
