...
/**
  * Returns MSI-X mask status for given interrupt vector number and virtual function
  * This should be called before send an interrupt using msix_vf_send
  * @param pcie_nr     - PCIe cluster number
  * @param vf_nr       - virtual function number (0 to 15)
  * @param vec_nr      - interrupt vector number
  * @return            - the status of the interrupt vector
  ...
  *                      1 - interrupt masked
  */
unsigned int msix_vf_status(unsigned int pcie_nr, unsigned int vf_nr, unsigned int vec_nr);

/**
  * Send a MSI-X for a VF for a particular table entry, and optionally mask the interrupt.
  * Note! : msix_vf_status must be called before calling this function to check mask status
  *         and read MSI-X configuration information.
  * 
  * @param pcie_nr     - PCIe cluster number
  * @param vf_nr       - virtual function number (0 to 15)
  * @param vec_nr      - interrupt vector number
  ...
  *                      1 - mask after sending
  */
void msix_vf_send(unsigned int pcie_nr, unsigned int vf_nr, unsigned int vec_nr, unsigned int mask_en);
